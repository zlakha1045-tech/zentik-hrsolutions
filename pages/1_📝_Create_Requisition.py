...
# --- PDF GENERATOR CLASS (Recreating your Template) ---
class MRF_PDF(FPDF):
    def header(self):
    ...
    def draw_form(self, data):
        ...
        self.cell(150, 10, data.get('requisitor_name', ''), border='RBT', ln=1)
        # Row 2
        self.cell(40, 10, "Department:", border='LBT')
        self.cell(80, 10, data.get('requisitor_dept', ''), border='BT')
        self.cell(30, 10, "Tel/Ext:", border='BT')
        self.cell(40, 10, data.get('requisitor_ext', ''), border='RBT', ln=1)
        self.ln(2)
        ...
        self.cell(80, 10, data.get('job_title', ''), border='BT')
        self.cell(20, 10, "Shift:", border='BT')
        self.cell(50, 10, data.get('shift', ''), border='RBT', ln=1)
        # Row 2
        ...
        # 4. CLASSIFICATION (Checkboxes simulated)
        self.section_title("CLASSIFICATION")
        
        # A. Recruitment Type
        rtype = data.get('recruitment_type', '')
        ...
        self.cell(0, 8, f"Reason: {data.get('replacement_reason', 'N/A')}", 0, 1)

        # B. Employment
        enature = data.get('employment_nature', '')
        ...
        self.multi_cell(0, 8, f"Education:\n{data.get('education_req', '')}", 1)
        self.multi_cell(0, 8, f"Skills:\n{data.get('skills_req', '')}", 1)
        self.multi_cell(0, 8, f"Experience:\n{data.get('experience_req', '')}", 1)
        self.ln(2)

        # 6. RESPONSIBILITIES
        self.section_title("AREAS OF RESPONSIBILITY")
        self.multi_cell(0, 8, data.get('responsibilities', ''), 1)
        self.ln(5)
...
with st.form("new_mrf_form"):
    ...
    # "Preview" is now part of the form flow
